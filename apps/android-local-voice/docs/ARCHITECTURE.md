...
世代遷移はfunction call eventの送信と直列化し、旧世代のMCP Jobを専用理由でcancelして通常のtool failureへ境界化してから新世代をackします。新世代のremote executorはack書き込み成功まで世代Deferredで待機し、失敗時は同じ世代を再適用せず、上限付き指数バックオフで再送します。
MCP接続とrouteは世代固有の不変catalogとして現行世代から隔離して準備し、全toolの取得とregistry commitが成功したときだけ切り替えます。commit失敗時は準備catalogを閉じ、明示的なtools置換時は退役catalogを閉じます。toolsを省略したinstructions-only更新だけは現行catalogを継承します。
MCP実行は承認・準備と副作用dispatchを分け、承認中は世代遷移によるcancelを受けられるようにします。dispatchは世代遷移と直列化し、先に遷移した旧世代callは破棄し、先にdispatchしたcallは完了してから次世代をackします。
